...
async def activity_detail(request: Request, act_id: str):
    ...
    # Calculate Efficiency Factor (EF) for the current activity
...
async def evaluate_activity(request: Request, act_id: str):
    ...
    # Calculate EF for current activity
    current_hr = act_data.get("average_heartrate")
...
"""
    ...
    prompt = f"""
あなたはプロのデータドリブンなサイクリングコーチです。
ユーザーから提供されたトレーニングデータを分析し、日本語で極めて的確で、モチベーションを高めるプロフェッショナルなフィードバックを作成してください。

【本日のアクティビティデータ】
- タイトル: {act_data.get('name')}
- 時間: {round(act_data.get('moving_time', 0)/60, 1)} 分
- TSS (トレーニング負荷): {act_data.get('tss')}
- 平均パワー: {metrics.get('average_power')} W (FTP: {ftp}W)
- 標準化パワー (NP): {metrics.get('normalized_power')} W
- 変動指数 (VI): {metrics.get('variability_index')}
- 有酸素デカップリング (心拍ドリフト率。％が低いほど有酸素運動の耐久力が高く良好。高いほど心肺が疲労しており悪化。通常5%未満が理想的): {f"{metrics.get('aerobic_decoupling_pct')}%" if metrics.get('aerobic_decoupling_pct') is not None else "データなし"}
- ケイデンス低下 (後半平均 - 前半平均。0rpmに近いほど後半も回せており良好。マイナスの絶対値が大きいほど後半に脚がタレているため悪化。例えば、-5.0 rpm は -0.1 rpm よりも大幅に悪い状態であることに注意してください): {f"{metrics.get('cadence_dropoff_rpm')} rpm" if metrics.get('cadence_dropoff_rpm') is not None else "データなし"}
