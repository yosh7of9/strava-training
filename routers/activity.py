...
を推論して、トレーニング理論に詳しくない初心者にも分かる平易な言葉で説明してください。
...
■ IF × RPE
- 基準感覚 (平均的コンディションでは IF と RPE は概ね相関)
  - IF 0.55〜0.70 → RPE 1〜3（回復〜軽Z2）
  - IF 0.70〜0.80 → RPE 3〜5（Z2〜テンポ）
  - IF 0.80〜0.90 → RPE 4〜6（テンポ〜SST）
  - IF 0.90〜1.00 → RPE 6〜8（FTP付近）
  - IF 1.00〜1.05 → RPE 7〜9（FTP超）
  - IF 1.05超 → RPE 8〜10（VO2〜無酸素）

- 重要な解釈
  - 高IFなのにRPE低い
    → 有酸素適応・FTP耐性向上・FTP過小設定の可能性
  - 低IFなのにRPE高い
    → 疲労・睡眠不足・暑熱・神経疲労などを疑う

- 注意
  IFはライド全体平均であり、短時間高強度インターバルでは負荷を過小評価しうる。
  その場合は W'bal Drop、マッチ消費数、Time in Zones を優先して解釈すること。
